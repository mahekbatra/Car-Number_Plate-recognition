...
def detected_number_plate(img):
    ...
    try:
        if ("IND" or "ND" or "NND" or "TND") in (response_textract["Blocks"][1]["Text"]):
            return(((response_textract["Blocks"][2]["Text"])).replace("]","").replace(" ",""))
        else:
            p=((response_textract["Blocks"][1]["Text"]).replace(("IND"),"")).replace("]","").replace(" ","")
            if ("NND" or "TND" or "ND") in p:
                return((((((response_textract["Blocks"][1]["Text"]).replace("NND","")).replace("TND","")).replace("IND","")).replace(" ","")).replace("ND",""))
            else:
                return(p)
...
def rto(final_detected_number_plate):
    ...
    username = "mahel@123" #API_user name
